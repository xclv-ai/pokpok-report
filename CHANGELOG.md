...
## TODO - In Progress
- [ ] 5 Truth prompt refactor — mirror AI GEO (Gemini 3 Flash) pattern: add <PRE_COMPUTED> echo contract (deterministic min/max/divergence/cohorts in n8n Code node), trim ~30%, save as v2_gemini-3-flash + example_output.json + pre_compute_node.js. Save-only; user deploys n8n. [TODO migrated from session-start fallback — admin_todos MCP unreachable]
...
- [x] UniversalDataProvider (Framer) — code component with property-controls UI panel, GitHub+Supabase data source picker. Live on /products/single-pdp-reports/brand-perception/brand-perception-truth via codeFileId `cqhovi4` (v1.7.0). 17 Evidence Item instances + The Battlefield Frontline (`bAmZ5rfrF`) all bind via DataOverrides.Data. Full interactive WWW analysis flow shipped 2026-04-22. [2026-04-21 → 2026-04-22]
- [ ] Brand Perception WWW V4 sandbox iteration — created copy `52lZstOsQMhgspl8` (WIP). Cross-brand evaluation across 4 URLs (tatcha/warbyparker/notion/allbirds) revealed 3/4 brands silently halted at `Clean relevant urls` due to hardcoded `tatcha.com` in `Relevant URLS` HTTP node body. Patches v3-v6 deployed: (v3) prompt LEGACY MAPPING + temp=0 on 3 Gemini nodes; (v4) parser stage redesign — single Firecrawl + Code post-processor (drops 3-way scrape + 2 Gemini merger calls); (v5) screenshot stack collapsed 6→2 active nodes (HTTP primary + ScrapingBee fallback); slim AI GEO context (~80KB→~5KB user prompt); (v6) added Merge + Combine HI+GEO Code node fusing 17-marker + AI/GEO outputs into single clean JSON `{brand, url, hi:{}, ai_geo:{}}`. Total nodes 82→64. Tatcha A/B PRE-PATCH 3/5 → POST-PATCH 4/5 on 17-marker quality. Per-brand reports + master report in `n8n/web analysis/new-workflows/runs/2026-04-26_v4-node-comparison/`.
- [x] Brand Perception WWW V3.3 — sitemap-driven brand analysis (workflow `VU4VKleSgaZZJWyH`, active). Split-architecture: Clean Sitemap regex-detects products vs narrative candidates (caps at 5 products, 200 narrative entries); Classify URLs (Gemini 2.5-flash, thinkingBudget=0, maxOutputTokens=8192) picks about/mission/vision/values/positioning/manifesto from narrative list; Fan Out 8 URLs normalizes + dedupes URLs (skips homepage — visual path covers it); Scrape Relevant Page pulls full markdown per URL; Aggregate emits full `brand_content` map (no truncation, no snippets — full markdown dominates prompt). Screenshot: 3-tier fallback (Firecrawl basic → Firecrawl Premium/stealth proxy → ScrapingBee) feeds `Analyze an image` via binary-mode Gemini. Prompts wraps content in `<BRAND_CONTENT_FULL_MAP>` + `<VISUAL_DATA_SOURCE>`. Tested on pokpok.ai, patagonia.com, liquiddeath.com — all 8 stages verified. Per-brand cost: small $0.15, large $0.34. Failures logged: F078 (n8n sandbox no URL ctor), F079 (Gemini imageUrls rejects data: URLs), F080 (3-tier screenshot fallback).
- [ ] Handwritten font — improve glyph vector tracing accuracy (5-phase plan: measurement infra → filled adaptive threshold → corner-aware smoothing → stroke v3 continuous paths → varstroke oriented ellipse)
...
- **fix:** n8n V3.10 (`Ca58L0pbclwDYwUw`) `Extract Data.user_id` was reading `data.user_id` (Polar customer_id) instead of `data.metadata.reference_id` (Supabase auth.uid). Every paid order since the V3.x rewrite stored the wrong `user_id`, causing OrderList (which queries `WHERE user_id = auth.user.id`) to silently filter rows out. Fix: change one expression on the live Extract Data node, published. All future orders write the correct uid.
- **fix:** DB backfill — deleted older pokpok.ai duplicate row (May 6, satisfies `(user_id, slug)` unique constraint) + updated 2 rows for `ceo@xclv.com` from Polar customer_id `bf774dc1-...` to Supabase auth.uid `c84ed39d-...`. Verified in DevTools — both `pokpok.ai` (May 7) and `dragonrouge.com` (May 8) rows now visible in OrderList.
- **feature:** Edge function `polar-checkout-www` v3 deployed (Supabase). Accepts `x-pokpok-env: sandbox|prod` header (mirrors `polar-customer-portal` v1.3.0 contract); picks per-env Polar checkout link from secrets `POLAR_WWW_CHECKOUT_LINK_PROD` / `_SANDBOX`; legacy fallback for prod (existing secret) keeps backwards-compat. Default = prod when no hint. Verified via DevTools curl matrix: no-hint → prod, prod → prod, sandbox → 500 expected (sandbox secret not yet set).
- **feature:** `WebhookButton_v4.2.0.tsx` (local, awaiting paste) — sandbox toggle drives `x-pokpok-env` HTTP header instead of picking a function name. Drops 2 dead Property Controls (`polarSandboxFunction` / `polarProductionFunction`) that pointed at phantom edge functions `polar-checkout-www-sandbox` (never deployed). One function for both envs.
- **feature:** `OrderList_v2.1.7.tsx` deployed + verified working — adds `notchColor` Property Control (Color picker) in canvas. Recolors the 4 corner-square notches on every chip in every Order row (VIEW REPORT button, date pill, PAID link). Implementation walks the rendered DOM post-render: finds `[data-framer-name="Rectangle 46"]` divs (canvas-set name, stable across builds) and sets `style.backgroundColor`. No Smart Component prop dependency.
- **fix:** `DashboardDataProvider_v3.5.0.tsx` (local, awaiting paste) — removes hardcoded `POLAR_ENV = "sandbox"` constant + `x-pokpok-env: "sandbox"` header. Was forcing the universal `polar-customer-portal` edge fn into sandbox-only lookup → prod customer 422 → 500 (visible in console twice per page load). Removing the hint lets edge fn use its built-in prod-first fallback.
- **fix:** `OrderList_v2.1.6.tsx` (local, superseded by v2.1.7) — drops redundant `x-pokpok-env: "prod"` header + `env: "prod"` body field from the polar-customer-portal fetch. Edge fn auto-routes — explicit hints were unnecessary.
- **improvement:** Verified Polar PROD + SANDBOX webhook URL via Polar API: both → `https://xclv.app.n8n.cloud/webhook/polar-paymentV2` (workflow `Ca58L0pbclwDYwUw` V3.10). Two other active polar-payment workflows (`w61btrWPdIEArbAd` V3.2 on `/webhook/polar-paymentV3` + `Xnl1Dc4UVWFlzuP2` original) are orphaned — Polar never calls them. Recommended deactivation pending user OK.
- **improvement:** Edge function `polar-checkout` v18 (ASIN flow) — local file written but deployment deferred. v17 has hardcoded sandbox URL, but no prod ASIN/Brand Alignment checkout link exists yet. Deploy with `POLAR_PDP_CHECKOUT_LINK_PROD` secret when ready.
- **improvement:** Failure registry updated — `pokpok-ai/.claude/skills/framer/references/failures.md` adds F105 (picked stale local file as base because version-string sort lied — should have used `ls -lt` mtime sort, or `readCodeFile` from Framer MCP) and F106 (proposed schema-level n8n change without auditing all readers first — must grep all readers before proposing).

> **Note:** Today's verified end-to-end flow:
> ```
> Polar PROD checkout → /webhook/polar-paymentV2 → Ca58L0pbclwDYwUw V3.10
>   → Extract Data (user_id = metadata.reference_id) → user+slug dedup
>   → Supabase Create → Route by product_type
>   → www_brand_perception → Check WWW Cache → cache hit → status=ready
>     OR cache miss → Trigger WWW Analysis (executeWorkflow → 52lZstOsQMhgspl8)
> ```
> See `diagnose/brand-perception/STATUS.md` for the full pipeline diagram + per-substep verification status. Polar customer cabinet integration uses dedicated `polar_customer_id` column (not `user_id`) so today's `user_id` schema change does not affect cabinet links.

---

## 2026-04-22

### Brand Perception Truth — interactive WWW analysis flow

- **feature:** `WebhookButton.tsx` v2.0.0 → v2.2.0 — new Framer code component replacing the legacy `Proceed to Checkout` submit button on `/products/single-pdp-reports/brand-perception/brand-perception-truth`. Reads URL from the form input, calls the `validate-www-url` edge function, POSTs `{www_link, slug, source}` to the n8n `brand-perception-www` webhook, and dispatches `pokpok-webhook-triggered` so the provider can re-fetch. Brand-compliant per pokpok-ui spec: yellow `#F5F900` bg + teal `#0A3B44` text, JetBrains Mono 14px, 4× 4×4px corner notches in parent bg color, hover flip. Button height locked to 50 × 100% width (immune to Framer's injected `height: 100%`).
- **feature:** Supabase edge function `validate-www-url` v2 — real reachability check, not just URL syntax. Auto-prepends `https://` for bare domains (`pokpok.ai`, `xclv.com`), rejects localhost/private ranges, performs HEAD/GET probe with 8s timeout, classifies `<400` valid / `404` invalid / `5xx` invalid / other 4xx valid (site up, just anon-blocked) / timeout+DNS fail invalid. Deployed to project `iyyuxilkacylpbweulsa`, verify_jwt=false.
- **feature:** `WwwFormOverrides.tsx` v1.0.0 → v1.1.0 — new Framer code file with two dedicated WWW-form overrides. `WwwUrlInput` debounces keystrokes 400ms, calls the edge function, broadcasts `pokpok-www-validation` events (states: idle / checking / valid / invalid). `WwwValidationBadge` toggles the Status component's visibility based on those events — hidden for empty / checking / valid, shown only when the edge function returned `valid: false`. Preserves the panel-set `content` prop (no text rewrite).
- **feature:** `UniversalDataProvider.tsx` v1.5.0 → v1.7.0 (codeFileId `cqhovi4`) — two additive bumps. v1.6.0 adds opt-in Supabase realtime subscription on `<realtimeTable>` filtered by `<realtimeUrlColumn>=eq.<watched>`, listens for `pokpok-webhook-triggered` to re-open the channel with the submitted URL, re-dispatches `pokpok-data-ready` on INSERT/UPDATE. v1.7.0 adds `Mirror WWW -> PDP` panel prop (default true) that writes the fetched www payload to the `pdp` slot — this fixed **The Battlefield Frontline** (nodeId `bAmZ5rfrF`) and any other `pdp_*`-bound component on WWW-only pages. All 17 Evidence Items + Battlefield section now render real Bramley brand-perception data, not placeholder labels.
- **fix:** WebhookButton v2.1.0 rejected legitimate short domains (`pokpok.ai` = 9 chars) because of a strict `length < 10` client preflight. v2.2.0 drops the length floor (keeps 500-char ceiling as sanity rail), auto-prepends `https://` for bare domains, delegates the real validation to the edge function.
- **fix:** Legacy "PASTE A VALID AMAZON PRODUCT LINK TO CONTINUE" copy on the WWW page — the `ValidationBadge` override from `Paywall/OrderFormProvider.tsx` was hardcoding Amazon-specific text even on this WWW-only page. Replaced with `WwwFormOverrides.WwwValidationBadge`; the original PDP override and file are untouched so the brand-alignment page keeps working.
- **improvement:** 5 edge-function test scenarios verified via curl and 4 end-to-end browser scenarios verified via Chrome DevTools (empty / junk / valid / unreachable). Battlefield Frontline live-DOM confirmed rendering `"Gentle Countryside Ritual"` + `"Nature's Gentle Wisdom"` + full category summaries from the Bramley JSON.

> **Note:** Realtime subscription plumbing is in place but not yet activated on the canvas — to enable auto-refresh after a submitted URL is processed, set `realtimeTable = www_perception` and `realtimeUrlColumn = url` on the UniversalDataProvider instance's panel. `OrderFormProvider.tsx` (PDP, frozen at `b0btPSD`) was not touched; brand-alignment page unchanged.
